...
- [x] Verify benchmarks compile + run, benchstat-compatible
      (`go test -run '^$' -bench ... -benchmem`) (PB-T3)

## 2. Steering rule (`openspec/config.yaml`)
- [x] Append **Rule 75 (No Networked pprof)**: profiling via Go built-in
...
## 4. Validation
- [x] Profile capture proven with no listener:
      `go test -run '^$' -bench Hash -cpuprofile /tmp/cpu.prof -memprofile /tmp/mem.prof`
      → valid pprof, `go tool pprof /tmp/cpu.prof` works (PB-T6)
- [x] `go vet` + `go test -race -cover` green for the touched packages (PB-T7)
- [x] `git diff master...HEAD --name-only` = bench files + OpenSpec set +
      config.yaml only (PB-T8)
- [x] Initial baseline numbers posted to issue #948 for Win1/2/3 (PB-T9)
