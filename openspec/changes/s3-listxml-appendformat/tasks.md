...
- [x] In `FormatListObjectsV2XML`: declare `var timeBuf [32]byte`; in the
...
- [x] `BenchmarkFormatListObjectsV2XML_AppendFormat`: `ReportAllocs`, 1000
      synthetic `common.FileMetadata`, call `FormatListObjectsV2XML` (LXF-T2)
- [x] `BenchmarkFormatListObjectsV2XML_OldFormat`: reference `time.Format` path
...
- [x] CI green including `benchstat` and `review`
