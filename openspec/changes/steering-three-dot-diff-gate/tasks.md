...
- [x] **Rule 55**: append pre-merge gate — verify `git diff master...HEAD --name-only`
...
- [x] **Rule 62**: replace two-dot `gh pr diff --name-only` with three-dot
      `git diff master...HEAD --name-only` in the staleness check (SDG-T2)
- [x] **Rule 73**: document that perf optimizations with a benchmark surface do
...
- [x] Add WP1 "merge-inflation" pitfall: use three-dot to enumerate PR's own
      changes after Rule 50 merge (SDG-T4)
- [x] Add WP2 "frozen gh pr checks" pitfall: cross-check `gh api .../jobs` when a
      check stays pending with frozen `updatedAt` (SDG-T5)

## 3. Reviewer script audit (Rule 70)
- [x] Confirm `.github/scripts/ai_reviewer.py` uses three-dot diff (lines 13, 161);
...
## 5. Validation
- [x] `git diff master...HEAD --name-only` on this PR shows only config.yaml, the
      AI_FLYING_SOLO.md pitfalls, and the OpenSpec change set (SDG-T1 re-check)
- [x] CI green including `review`
