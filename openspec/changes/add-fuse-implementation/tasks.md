...
- [ ] 1.1 Review current `src/momofs/fuse.go` — document all 22 callback signatures and their usage sites
- [ ] 1.2 Audit `consistency=cached` usage across config and code — identify all 8+ references
- [ ] 1.3 Inventory platform targets: Linux (bare-metal/Docker), macOS (Docker Desktop), Windows (if any)
- [ ] 1.4 Decide on go/no-go for go-fuse/v2 migration vs VirtioFS-only

## Phase 2: go-fuse/v2 Adapter (3-5 days)
- [ ] 2.1 Fork/adapt `hanwen/go-fuse/v2` to provide the 22 FUSE callback adaptations
- [ ] 2.2 Map each prior callback to go-fuse/v2 equivalent (see spec.md mapping table)
- [ ] 2.3 Implement splice/ReadResultPipe for zero-copy read path where kernel supports it
- [ ] 2.4 Replace `src/momofs/fuse.go` with adapter code; ensure `go build ./...` succeeds
- [ ] 2.5 Write unit tests for each adapted callback (happy path + error path)

## Phase 3: consistency=cached Deprecation (1-2 days)
- [ ] 3.1 Add deprecation AUDIT log in config reload path when `consistency = "cached"` is set
- [ ] 3.2 Remove `consistency` from the public config schema (keep internal only with warning)
- [ ] 3.3 Update `docs/TESTING.md` with deprecation test results
- [ ] 3.4 Verify no behavioral change when flag is set/unset
...
- [ ] 5.1 Build momofs on Linux with go-fuse/v2 — confirm 0 compile errors
...
- [ ] 5.5 Run integration test: mount, 1000 upload/download cycles, data integrity verify
