...
## 2026-06-15 - Path Traversal via Unsanitized Cryptographic Hash
**Vulnerability:** The application used an untrusted, client-provided cryptographic hash (e.g., from network protocols) directly as part of a file path in the CAS storage backend (`getBlobPath`), prior to any hash computation and verification. By providing a malicious hash like `../../../some/file`, an attacker could traverse directories and read, overwrite, or delete arbitrary files during the `store.Has()` check or subsequent storage operations.
**Learning:** Even fields labeled as "Hashes" or other cryptographic identifiers cannot be trusted if they originate from unauthenticated or external client inputs. Treating them as safe identifiers before verifying them allows path traversal and command injection vulnerabilities.
**Prevention:** Always validate and sanitize untrusted client-provided identifiers (including hashes) against directory traversal characters (`.`, `/`, `\`) immediately upon receipt and strictly *before* passing them to any storage backend operations like `store.Has()`.
