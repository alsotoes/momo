...
## 2026-05-30 - CRLF Log Forging via Unsanitized Input
**Vulnerability:** The application logged untrusted string data (e.g., `remoteAddr`, `err.Error()`) directly into its `AUDIT` logs using `log.Printf`. An attacker could inject carriage returns (`\r`) or line feeds (`\n`) within this data to forge fake log entries (Log Forging/CRLF Injection), corrupting the integrity of the audit logs.
**Learning:** Raw input must never be directly interpolated into structured or audit logs. Even if a string like an IP address or error message seems safe, an attacker controlling the network payload or file paths can supply malicious values with newlines.
**Prevention:** Always sanitize untrusted input before logging by escaping or stripping control characters (e.g., `\r` and `\n`) using a centralized helper function like `momo_common.SanitizeLog`.
