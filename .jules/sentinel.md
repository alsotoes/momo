...
## 2026-09-01 - Protocol Smuggling (CRLF Injection) in Internal Query Handler
...
## 2026-10-15 - Missing Path Traversal Validation in LocalBlobStore SyncBlob/SyncDir
**Vulnerability:** `LocalBlobStore.SyncBlob` and `SyncDir` constructed file paths from the provided `hash` parameter without sanitization, allowing an attacker to supply a hash like `../../../etc/passwd` to fsync arbitrary files or directories on the host system.
**Learning:** While the higher-level `CASStore` correctly validated hashes during `Put` operations, the underlying `DurabilityOps` interface methods lacked independent validation. Since `DurabilityOps` is a public interface, other components or future code could call `SyncBlob` or `SyncDir` with unvalidated input, bypassing the `CASStore` protections.
**Prevention:** Always implement defense-in-depth by validating input at the lowest possible layer (e.g., using `common.HasPathTraversalChars`) before constructing filesystem paths, even if a higher-level caller is expected to sanitize the input.
