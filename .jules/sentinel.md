...
## 2025-03-26 - Denial of Service via Hanging Outbound TCP Connections
**Vulnerability:** Unbounded outbound TCP connections using `net.DialTCP` (without timeouts). An attacker could exhaust system resources (file descriptors, memory) by causing outbound network calls to block indefinitely on unresponsive endpoints (Denial of Service).
**Learning:** All network communication—both incoming and outgoing—must use proper bounds. A connection attempt to another node might hang forever if the remote server drops packets without answering.
**Prevention:** Always use `net.DialTimeout` with a reasonable deadline (e.g., 10 seconds) instead of the default `net.Dial` or `net.DialTCP` for network client dial operations.
...
## 2025-03-27 - Missing Authentication on Protocol Endpoints
**Vulnerability:** The application's network daemon (`Daemon` and `ChangeReplicationModeServer`) endpoints processed all incoming protocol operations directly without requiring an authentication token. This lack of access control allowed unauthenticated external attackers to arbitrarily modify the server's replication state or send potentially malicious file uploads (Unauthorized Access / Missing Authentication).
**Learning:** Any network service exposed to external connections must enforce authentication bounds at the very beginning of the connection lifecycle before any application-specific payload or metadata is parsed or processed.
**Prevention:** Always enforce an authentication handshake (like a pre-shared `AuthToken`) immediately upon a TCP connection being established to sensitive endpoints, and terminate the connection before parsing any protocol data if the authentication is invalid.
