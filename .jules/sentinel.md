...
## 2026-05-31 - Log Injection via CRLF in Error and Network Input
**Vulnerability:** The application logged unverified external input (`conn.RemoteAddr().String()`) and wrapped error messages directly into `log.Printf`. An attacker could inject CRLF characters (`\r\n`) within the network address or triggered errors to forge arbitrary log entries, evading log monitoring or creating false audit trails.
**Learning:** `log.Printf` and similar standard logging functions do not inherently strip or encode newline characters in Go. Any untrusted string passed to them can break log structuring and create log forgery vulnerabilities.
**Prevention:** Implement a central sanitization function (e.g., `SanitizeLog` replacing `\r` and `\n`) and rigorously apply it to all externally sourced or untrusted data (like IP addresses, file paths, or complex error strings) before passing them to logging sinks.
