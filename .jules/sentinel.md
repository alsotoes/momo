...
## 2026-04-30 - DoS via Hanging Outbound Connections
**Vulnerability:** The application used `net.DialTimeout` for outbound network connections but failed to enforce any rolling read/write timeouts on the resulting connection. An attacker or a malfunctioning network node could accept the connection but hang indefinitely during data transfer, exhausting local file descriptors and causing a Denial of Service (DoS).
**Learning:** Establishing an initial connection with a timeout (`net.DialTimeout`) does not protect against an endpoint that connects but refuses to read or write data. Network endpoints must wrap established connections with rolling read/write deadlines to prevent slowloris-style resource exhaustion during active transfers.
**Prevention:** Always wrap outbound connections in an `IdleTimeoutConn` (or similar mechanism) that calls `SetReadDeadline` and `SetWriteDeadline` on every successful read or write operation to ensure the connection fails fast if the remote endpoint stalls.
