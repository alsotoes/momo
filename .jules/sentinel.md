...
**Vulnerability:** The application logged untrusted string data (e.g., `remoteAddr`, `err.Error()`) directly into its `AUDIT` logs using `log.Printf`. An attacker could inject CRLF characters (`\r\n`) within this data to forge arbitrary log entries, evading log monitoring or creating false audit trails.
**Learning:** Standard logging functions do not inherently strip or encode newline characters. Raw input must never be directly interpolated into structured logs; an attacker controlling the input can break log formatting and corrupt audit integrity.
