...
## 2026-05-20 - Slowloris DoS during Handshake\n**Vulnerability:** A slowloris attack was possible because no strict absolute deadline was applied before processing the handshake AuthToken, Timestamp, and Metadata. A rolling idle timeout is insufficient.\n**Learning:** When using idle rolling timeouts for transferring data, a strict absolute deadline must still be applied during the authentication and handshake phase, and then recalculate/extended based on size for the file transfer phase.\n**Prevention:** Enforce a short, absolute deadline (e.g. 10s) immediately upon connection acceptance, and explicitly replace or extend it once the payload size is known.
