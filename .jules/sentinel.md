...
**Prevention:** Always validate combined/wire file names against traversal characters (using `common.HasPathTraversalTokens` on the composite path) directly on the `wireName` in all transport protocols.

## 2026-07-30 - Path Traversal in S3 Communicator `SendMetadata`
**Vulnerability:** The `S3Communicator.SendMetadata` method constructs an HTTP PUT request using user-provided `RemotePath` and `Name` metadata fields. While it checks for carriage returns/line feeds to prevent header injection, it missed checking for path traversal characters (`..`, `/`, `\`) in the combined `wireName`. This allows constructing PUT paths containing traversal instructions.
**Learning:** Security validations were partially applied (CRLF) but a defense-in-depth approach is required for all untrusted data forming paths. The application has a centralized helper function `common.HasPathTraversalChars` designed precisely for this but it wasn't used in this specific method prior to constructing the wire format path.
**Prevention:** Always validate constructed network resource identifiers (like S3 paths) using central validation libraries against path traversal before they are serialized into HTTP requests or database keys.
