...
## 2026-07-18 - Missing upper bounds on S3 max-keys parameter
**Vulnerability:** DoS (Memory Exhaustion) via unbounded `max-keys` in S3 `ListObjectsV2`.
**Learning:** When parsing query parameters that control response sizes or loop iterations, failing to enforce an upper bound can allow malicious actors to request massive payloads, leading to excessive memory allocation or CPU usage on the server.
**Prevention:** Always proactively clamp user-provided limit parameters (like `max-keys`) to a safe, domain-appropriate upper bound (e.g., 1000) immediately after parsing.
