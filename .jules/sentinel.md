...
## 2026-05-03 - Denial of Service via Missing Read/Write Deadlines on Outbound Client Connections
**Vulnerability:** Client code using `common.DialSocket` initiated outbound TCP connections without configuring any ongoing read/write deadlines on the returned connection. A malicious or stalled server could cause client connections to hang indefinitely while attempting to read or write data, exhausting client resources.
**Learning:** `net.DialTimeout` only applies a deadline to the initial TCP handshake connection attempt. Once the connection is established, standard `Read` and `Write` calls will block forever unless a deadline is explicitly set on the connection itself using `SetDeadline`, `SetReadDeadline`, or `SetWriteDeadline`.
**Prevention:** Wrap outbound connections in a deadline-enforcing wrapper (like `IdleTimeoutConn` which resets deadlines on activity) or manually call `SetDeadline` before performing I/O operations to ensure unresponsive peers cannot tie up local threads and descriptors.
