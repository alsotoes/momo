...
## 2026-08-16 - Path Traversal via Unsanitized Hashes in RawBlobStore
**Vulnerability:** The `RawBlobStore` implementation for reading, writing, and deleting blobs from the raw block device accepted a `hash` string without checking for path traversal sequences like `../../../`.
**Learning:** Even though `RawBlobStore` doesn't write to standard filesystem files (it uses a BoltDB allocation table mapping hashes to raw block offsets), the `hash` is still used as a database key. While this doesn't directly allow escaping to the underlying OS filesystem, accepting unsanitized hashes is a logic bypass that could allow attackers to manipulate DB entries or bypass content-addressable constraints, violating defense-in-depth principles.
**Prevention:** Always sanitize hashes and input parameters used for data retrieval or storage operations at the lowest possible implementation layer (e.g., using `common.HasPathTraversalChars`) regardless of the underlying storage backend.
