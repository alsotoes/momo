...
## 2026-05-22 - Slowloris Bypass in Handshake Phase via Rolling Timeouts
**Vulnerability:** A rolling timeout (`IdleTimeoutConn`) was applied to network connections to prevent slow peers from holding them open indefinitely. However, an attacker could still conduct a Slowloris-style attack during the initial handshake (reading the AuthToken and Metadata) by trickling bytes very slowly to keep the rolling timeout active without actually completing the request.
**Learning:** Rolling timeouts alone are insufficient to secure the initial connection setup because they reset on every successful byte read. The handshake phase requires a strict, absolute deadline to ensure the connection transitions to a valid state within a predictable time limit, before transferring large amounts of data.
**Prevention:** In addition to rolling idle timeouts, establish a strict absolute deadline (`SetAbsoluteDeadline`) immediately upon accepting a connection to govern the maximum allowable time for the authentication and metadata handshake.
