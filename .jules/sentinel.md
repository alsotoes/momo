...
## 2026-07-26 - Path Traversal via Unsanitized Scatter-Gather Metadata
**Vulnerability:** The `StorageQueryHandler` processed internal scatter-gather queries (`handleHas`, `handleGet`, `handleDelete`) and decoded metadata lists (`DecodeFileMetadataList`) without sanitizing network inputs (hashes and filenames). An attacker compromising a peer node could inject path traversal characters (`..`, `/`, `\`) into these internal queries, bypassing frontend sanitization to interact with arbitrary files on the local filesystem or exhaust resources via malicious `Size` values.
**Learning:** Defense-in-depth requires sanitization at all boundaries, including internal cluster communication. Trusted peer networks (like scatter-gather nodes) cannot be implicitly trusted with raw data, as a single compromised node can propagate exploits to the rest of the cluster via internal RPCs.
**Prevention:** Always enforce path traversal and limit boundary sanitization (e.g., `common.HasPathTraversalChars`) on all incoming data fields (Hashes, Names, Sizes) at the exact point of processing or decoding, even if the data originates from a supposedly trusted internal cluster node.
...
**Learning:** Checking path traversal characters strictly at the transport or boundary layer before transmission ensures that unvalidated inputs are rejected early. However, when validating combined path/name fields (`wireName`), naive validation against directory separators like `/` will trigger false positives for legitimate virtual directories. The path must be split into its constituent segments (using `strings.Split(wireName, "/")`) and each individual segment validated against path traversal characters to allow virtual directories while strictly blocking parent traversal sequences (`..`).
**Prevention:** Always validate combined/wire file names against traversal characters (using `common.HasPathTraversalChars` on each path segment) directly on the `wireName` in all transport protocols.
## 2026-08-05 - Missing Path Traversal Validation in LocalBlobStore Reads/Deletes
**Vulnerability:** `LocalBlobStore.GetBlob` and `DeleteBlob` blindly constructed file paths from the provided `hash` parameter without sanitization, allowing an attacker to supply a hash like `../../../etc/passwd` to read or delete arbitrary files on the host system.
**Learning:** While the higher-level `CASStore` correctly validated hashes during `Put` operations, the underlying `BlobStore` interface methods lacked independent validation. Since `BlobStore` is a public interface, other components or future code could call `GetBlob` or `DeleteBlob` with unvalidated input, bypassing the `CASStore` protections.
**Prevention:** Always implement defense-in-depth by validating input at the lowest possible layer (e.g., using `common.HasPathTraversalChars`) before constructing filesystem paths, even if a higher-level caller is expected to sanitize the input.
...
## 2026-08-30 - Missing Connection Close on Shutdown Semaphore Timeout
**Vulnerability:** In `src/server/server.go`, a `select` statement was used to wait for a slot in the concurrent connection semaphore (`sem`). If the context was canceled (`ctx.Done()`) while waiting, the function returned `nil` but failed to close the accepted `connection`.
**Learning:** This could lead to a resource leak (open connection/file descriptors) during server shutdown if the concurrency limit is reached and the server attempts to gracefully exit. The counterpart logic in `src/server/replication.go` had already been refactored to use `acquireConnectionSlot` and correctly closed the connection, but this was missed in the main `Daemon` accept loop.
**Prevention:** When accepting new connections, always ensure the connection is closed in all error or early-return paths (like context cancellation) before the connection's dedicated goroutine is spawned and takes ownership.
## 2026-09-02 - Path Traversal in Metadata Uploads
**Vulnerability:** The metadata upload validation logic in `momo_tcp`, `momo_quic`, and `s3_communicator` was splitting the `wireName` by `/` and passing each part to `common.HasPathTraversalChars()`. This completely missed path traversals like `../` because `HasPathTraversalChars` correctly returns `true` only if the exact string matches `..` or contains `/` or `\`. Thus, splitting the string by `/` removed the very characters that needed to be found.
**Learning:** Checking for directory traversal by splitting on path separators defeats the purpose of the validation function if the validation function itself looks for those very path separators.
**Prevention:** Always validate the combined virtual path directly using `common.HasPathTraversalChars` before transmitting files, rather than splitting it.
