...
## 2026-06-14 - Fix Path Traversal in S3 Communicator
**Vulnerability:** The S3 Communicator's HandshakeServer and ReceiveMetadata methods only used strings.TrimPrefix on incoming URL paths without verifying if the path contained upward traversal payloads (e.g., ../).
**Learning:** S3 object keys can validly contain slashes to simulate directories, so strict slash blocking is a functional regression. However, failing to resolve relative path segments allows attackers to read or write to arbitrary locations via URL paths.
**Prevention:** Use path.Clean on the parsed path and explicitly reject any output that equals ".", "..", or starts with "../" to permit valid S3 keys while thwarting traversal.
