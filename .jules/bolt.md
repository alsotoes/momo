...
## 2026-05-21 - [Consolidate Network Writes Using Stack Arrays Safely]
**Learning:** In Go, repeatedly calling `conn.Write` or using `json.NewEncoder` for small payloads (like authentication tokens + JSON structs) causes unnecessary memory allocations and system call overhead. When combining multiple network payloads into a single buffer to save system calls, using `copy()` into a fixed-size stack array without checking lengths can cause silent truncation if the payload exceeds the buffer size (dangerous for dynamic JSON).
**Action:** Consolidate network writes by marshalling JSON first and appending it to a dynamic slice backed by a small stack array (`var stackBuf [128]byte; payload := append(stackBuf[:0], ...)`). This achieves zero-allocation performance for small payloads while automatically falling back to safe heap allocation if the payload grows unexpectedly, preventing truncation bugs and halving the number of write system calls. Also, manually append a trailing newline `\n` to the payload when replacing `json.NewEncoder` to maintain wire-protocol compatibility.
