...
## 2026-09-24 - Zero-allocation string parsing
**Learning:** Using `strings.Split` in high-throughput hot paths like authentication headers creates unnecessary heap allocations, increasing GC pressure. Replacing standard library split functions with manual parsing loops using `strings.Index`, `strings.IndexByte`, and slice indexing eliminates these allocations completely.
**Action:** Always prefer manual slice-based parsing over `strings.Split` for critical, high-frequency network paths.
