...
## 2026-06-13 - [Eliminate Heap Allocations from binary.Write in Hash Loops]
**Learning:** `binary.Write` uses reflection internally, which causes variables to escape to the heap, resulting in an allocation and garbage collection overhead. This is especially impactful in high-frequency loops like CRUSH map placement hash calculations. Similarly, converting a string parameter directly to `[]byte(str)` before a `hash.Write` causes an unnecessary allocation compared to `io.WriteString(hash, str)`.
**Action:** Replace `binary.Write` with direct buffer assignments (e.g., `var buf [4]byte; binary.LittleEndian.PutUint32(buf[:], val)`) and subsequent writes. Replace `hash.Write([]byte(str))` with `io.WriteString(hash, str)` to eliminate these allocations entirely and improve micro-benchmark performance.
