...
## 2024-05-18 - [Optimize SigV4 Authentication Parsing]
**Learning:** High-throughput request authentication paths (like SigV4 `parseSigV4AuthHeader` and `parseSigV4QueryAuth`) are prime targets for zero-allocation optimizations. Standard library functions like `strings.Split` and `strings.TrimSpace` incur significant heap allocation overhead (due to dynamic slices and string copies) when executed on every request.
**Action:** Replace heap-allocating string manipulation functions on hot paths with manual, zero-allocation parsing loops using stack-allocated arrays (e.g., `var credParts [5]string`), direct slice indexing, and byte index lookups (e.g., `strings.IndexByte`). Always measure the impact using Go benchmarks with `-benchmem` to ensure allocations are reduced or eliminated without sacrificing correctness.
