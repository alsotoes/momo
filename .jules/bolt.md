...
## 2024-05-19 - [Consolidating Network Writes Using Stack Arrays Safely]
**Learning:** When attempting to combine multiple network payloads into a single buffer to save system calls, using `copy()` into a fixed-size stack array (e.g., `var buffer [256]byte`) without checking lengths can cause silent truncation if the combined payload exceeds the buffer size. This is particularly dangerous with dynamic JSON payloads.
**Action:** Instead of blind `copy()`, use a dynamic slice backed by a small stack array (`var stackBuf [128]byte; payload := append(stackBuf[:0], ...)`). This achieves zero-allocation performance for small payloads while automatically falling back to safe heap allocation if the payload grows unexpectedly, preventing truncation bugs. Additionally, when replacing `json.NewEncoder(conn).Encode(data)` with `json.Marshal`, manually append a trailing newline `\n` to the payload to maintain exact wire-protocol compatibility.
