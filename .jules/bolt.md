...
## 2026-05-15 - Testing with Consolidated JSON Network Payloads
**Learning:** When network payloads are consolidated to reduce system calls, appending a trailing newline (e.g. `\n`) after JSON data is common and required for streaming decoders (`json.NewDecoder`). However, this breaks unit tests that expect an exact string match without accounting for the added newline character.
**Action:** When validating raw bytes sent over a connection that include consolidated JSON, ensure test validations trim both null bytes (padding) and the appended newline (e.g., using `strings.TrimRight(string(data), "\x00\n")`) to prevent spurious assertion failures.
