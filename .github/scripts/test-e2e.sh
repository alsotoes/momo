...
# Usage: ./test-e2e.sh [momo-tcp|momo-quic]
PROTOCOL=${1:-"momo-tcp"}

echo "Building Momo for E2E tests ($PROTOCOL)..."
make build

echo "Setting up local directories..."
E2E_DIR="/tmp/momo-e2e-$PROTOCOL"
rm -rf $E2E_DIR
...
# Use the momo binary itself to change replication mode in a transport-agnostic way
./bin/momo -imp repl -mode 1 -config $E2E_DIR/e2e.conf > $E2E_DIR/repl.log 2>&1
sleep 3

# Create dummy file to send
echo "e2etestdata-$PROTOCOL" > $E2E_DIR/test_e2e_file.txt
...
echo "Data is consistent across all servers. E2E Test Passed ($PROTOCOL)!"
