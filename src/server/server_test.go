...
func handleConnection(t *testing.T, connection net.Conn, daemons []*momo_common.Daemon, serverId int, authToken string) {
	...
	if _, err := connection.Read(bufferAuthToken); err != nil {
...
func TestDaemonLogic(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			go func() {
				handleConnection(t, server, daemons, tc.serverId, "test_token")
				close(serverDone)
			}()

			// Test Execution
			client.Write([]byte(momo_common.PadString("test_token", 64)))
