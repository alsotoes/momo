...
func Daemon(ctx context.Context, cfg common.Configuration, serverId int) error {
	...
	for {
		...
		go func(comm transport.Communicator) {
			...
			// 🛡️ Sentinel: Sanitize Hash to prevent path traversal in CAS storage
			if strings.Contains(metadata.Hash, ".") || strings.Contains(metadata.Hash, "/") || strings.Contains(metadata.Hash, "\\") {
