...
// TestMomoTCPReceiveMetadata_RejectsPathTraversal verifies issue #997:
...
func TestMomoTCPReceiveMetadata_RejectsPathTraversal(t *testing.T) {
	tests := []struct {
		...
		{name: "valid 64-char hex hash accepted", hash: strings.Repeat("a", 64), fileName: "file.txt"},
		{name: "dot-dot traversal rejected", hash: "..", fileName: "file.txt", wantErr: true},
		{name: "forward slash traversal rejected", hash: "dir/hash", fileName: "file.txt", wantErr: true},
		{name: "backward slash traversal rejected", hash: "dir\\hash", fileName: "file.txt", wantErr: true},
		{name: "relative parent traversal rejected", hash: "../../../etc/passwd", fileName: "file.txt", wantErr: true},
		{name: "empty hash rejected", hash: "", fileName: "file.txt", wantErr: true},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				if err == nil {
					t.Fatal("expected ReceiveMetadata to reject path traversal in hash")
				}
				if !errors.Is(err, syscall.EBADMSG) {
					t.Fatalf("expected EBADMSG, got: %v", err)
				}
