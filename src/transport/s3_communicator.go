...
func (m *S3Communicator) HandshakeServer(expectedAuthToken []byte) (requestedMode int, timestamp int64, err error) {
	...
	if req.Method == "GET" {
		...
		if isList {
			...
			m.conn.SetWriteDeadline(time.Now().Add(5 * time.Second))
			// ⚡ Bolt: Eliminate bytes.Buffer heap allocations and intermediate slice copies
			// by using a stack-allocated byte array and append/strconv.AppendInt for HTTP headers.
			// This reduces GC pressure significantly for high-frequency S3 List requests.
			...
			if _, err := m.conn.Write(xmlBytes); err != nil {
				return 0, 0, fmt.Errorf("failed to write XML list response body: %w", err)
			}
		...
		// ⚡ Bolt: Eliminate bytes.Buffer heap allocations for HTTP response headers
		// using a pre-allocated stack buffer. Avoiding memory allocations in hot paths
		// prevents GC pauses and CPU overhead.
		var headerBuf [256]byte
		b := headerBuf[:0]
		b = append(b, "HTTP/1.1 200 OK\r\nContent-Length: "...)
