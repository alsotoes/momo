...
func FormatListObjectsV2XML(bucketName, prefix, delimiter string, maxKeys int, files []common.FileMetadata) []byte {
	...
	buf.Write(strconv.AppendInt(numBuf[:0], int64(maxKeys), 10))
	buf.WriteString(`</MaxKeys>`)
	...
	for _, file := range files {
		...
		buf.Write(strconv.AppendInt(numBuf[:0], file.Size, 10))
		buf.WriteString(`</Size>`)
	...
	buf.Write(strconv.AppendInt(numBuf[:0], int64(keyCount), 10))
	buf.WriteString(`</KeyCount>`)
