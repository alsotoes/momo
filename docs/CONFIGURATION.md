...
-   **`oprf_enabled`**
    ...
    -   **Protocol availability (S3 limitation):** OPRF confidential dedup is implemented only on the **native** transports, `momo-tcp` and `momo-quic` (native `ModeOPRFEval` / `SendOPRFEval`). The S3 gateway transports (`s3-tcp`, `s3-quic`) are served by `S3Communicator`, which does **not** implement OPRF evaluation — `SendOPRFEval` returns `ENOTSUP`. Setting `oprf_enabled = true` with `protocol = s3-tcp` or `s3-quic` **fails closed at upload/download**: the client aborts with `ENOTSUP` rather than falling back to the tenant content key. E2EE **encryption** itself (AES-GCM-256, tenant- and at-rest-derived keys) remains fully available on all four protocols; only OPRF dedup is unavailable on `s3-*`, so on those transports keep `oprf_enabled = false`.
    -   **Type:** Boolean (`true` or `false`)
