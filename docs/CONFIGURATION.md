...
-   **`oprf_enabled`**
    ...
    -   **Protocol availability:** OPRF confidential dedup is available on **all four** transports. The native protocols (`momo-tcp`, `momo-quic`) use the binary `ModeOPRFEval` (`'O'`) handshake; the S3 gateway (`s3-tcp`, `s3-quic`) serves the dedicated `POST /?momo-oprf-eval` HTTP endpoint (issue #817) with the same evaluation wire layout. `S3Communicator.SendOPRFEval` drives that endpoint, so `oprf_enabled = true` behaves identically regardless of `protocol`. Requires `[p2p]` enabled when `oprf_threshold > 1` (peers evaluate their Shamir shares via P2P). An S3 client that cannot perform OPRF evaluation (e.g. a stock `aws-cli` upload with `oprf_enabled`) still fails closed rather than silently falling back to a determinable content key.
    -   **Type:** Boolean (`true` or `false`)
